...
## MM468
scRNA_pers = readxl::read_xlsx(file.path(maindir, "output","scRNAseq","MM468","Persister","Supervised","Tables","Differential_analysis_Limma_logFC_1.58.xlsx"),sheet = 3)
# scRNA_UNC = readxl::read_xlsx(file.path(maindir, "output","scRNAseq","MM468","UNC","Supervised","Tables","Differential_analysis_Limma_logFC_1.58.xlsx"),sheet = 3)
scChIP_K27 = readxl::read_xlsx(file.path(maindir, "output","scChIPseq","ChromSCape_analyses","MM468_H3K27me3_peaks",
                                         "Diff_Analysis_Gene_Sets","DA_grouped_Persister_vs_DSMO_with_bulk_TSS.xlsx"),sheet = 3)
bivalent_df = read.csv(file.path(maindir, "output","bulk_ChIPseq","MM468","ChIPreChIP","fisher_pvalue_K27_K4_peak_0.001_0.15.csv"))
...
# TSS-based annot overlapping K27 peaks 
...
scRNA_pers$K27_status = "Not Overlapping"
scRNA_pers$K27_status[which(scRNA_pers$Symbol %in% annot10k_K27_ovlp$gene)] = "Overlapping"

scChIP_K27_byGene = scChIP_K27 %>% group_by(Gene) %>%
    slice_max(abs(log2FC.Persister))

all_genes = (union(scChIP_K27_byGene$Gene,scRNA_pers$Symbol))
length(all_genes)

combined_table = tibble(Gene = all_genes)
combined_table = left_join(combined_table, scRNA_pers[,c(1,4,7,12)], by = c("Gene"="Symbol"))
...
colnames(combined_table) = c("Gene", "log2FC.scRNA.Persister","qval.scRNA.Persister", "K27_status",
                             "log2FC.scChIP.H3K27me3","qval.scChIP.H3K27me3",
                             "OddRatio_bivalent", "q.value_bivalent", "Significantly_bivalent")
combined_table$qval.scRNA.Persister = as.double(combined_table$qval.scRNA.Persister)
combined_table$log2FC.scChIP.H3K27me3 = as.double(combined_table$log2FC.scChIP.H3K27me3)
WriteXLS(combined_table, ExcelFileName = file.path(maindir, "output","combined_DA_MM468.xlsx"))
