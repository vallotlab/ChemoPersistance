...
# Directories -------------------------------------------------------------
outdir = file.path(maindir, "output","scRNAseq", "PDX_BC1224");  if(!dir.exists(outdir)){dir.create(outdir)}
...
scRNA = list()
for(file in list.files(GEOdir_scRNA_MM468,pattern = "BC1224.*barcodes.tsv.gz", full.names = TRUE)){
...
for(i in seq_along(scRNA)){
    ...
    umi$filter_by_expr_features <- (umi$detected<high_th & umi$detected>low_th)
    #Distribution of number of total number of counts
    umi$filter_by_lib_size <- (umi$sum < library_size_high_th)
    #MT contamination
    umi$filter_by_MT_conta <- (umi$subsets_mt_percent<15)
    #Spike-in
    umi$filter_by_spikein <- (umi$subsets_ercc_percent<0.05)
    
    cat("Kept after filtering by expression features (%)", 100*table(umi$filter_by_expr_features)[2]/ncol(umi),"\n")
    cat("Kept after filtering by total counts (High) (%)", 100*table(umi$filter_by_lib_size)[2]/ncol(umi),"\n")
    cat("Kept after filtering by MT contamination (%)", 100*table(umi$filter_by_MT_conta)[min(length(table(umi$filter_by_MT_conta)),2)]/ncol(umi),"\n")
    cat("Kept after filtering by Spike in contamintion (%)", 100*table(umi$filter_by_spikein)[2]/ncol(umi),"\n")
    ...
    print(scater::plotColData(umi,x="detected",y="subsets_mt_percent"))
    print(scater::plotColData(umi,x="detected",y="subsets_ercc_percent"))
    hist(umi$detected[which(umi$detected>500)],breaks=100,main=" Number of detected genes",xlab="NUMBER of GENES")
    abline(v=high_th,col="red")
    abline(v=low_th,col="red")
    hist(umi$sum[which(umi$sum>5000)],breaks=100, main="Library size",xlab="TOTAL COUNTS")
    M <- median(umi$total_counts)
    abline(v=library_size_high_th,col="red")
    dev.off()
