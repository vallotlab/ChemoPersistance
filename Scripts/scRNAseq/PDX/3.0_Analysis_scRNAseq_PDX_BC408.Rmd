...
```{r paths}
#Paths
resdir <- file.path(here(),"output","scRNAseq","PDX_BC408")
...
resdir_BC976 <- file.path(here(),"output","scRNAseq","PDX")
RDatadir_BC976 = file.path(resdir_BC976, "Unsupervised", "RData")
```

```{r loads}
load(file.path(RDatadir,"PDX_BC408.RData"))
...
# Bind Signal & annots 
# genes_in_common = intersect(rownames(BC976$Signal), rownames(Signal))
# Signal = Signal[match(genes_in_common,rownames(Signal)),]
# BC976$Signal = BC976$Signal[match(genes_in_common,rownames(BC976$Signal)),]
# Signal = cbind(BC976$Signal, Signal)
# 
# gene_metadata = gene_metadata[match(genes_in_common,rownames(gene_metadata)),]
# annot = rbind(BC976$annot,annot)
...
rownames(Signal) <- gene_metadata$Symbol
annot$sample_id = gsub(" ","",annot$sample_id)
annot$cell_id = gsub(" ","",annot$cell_id)
annot_int = annot

rownames(annot_int) <- annot_int$cell_id
rownames(gene_metadata) <- gene_metadata$Symbol

cds <- new_cell_data_set(Signal,
                         cell_metadata = annot,
...
annot_int$cell_cycle = annot_seurat$cell_cycle[match(annot_int$cell_id, gsub(" ","",annot_seurat$cell_id))]
annot_int$Seurat_cluster = annot_seurat$Seurat_cluster[match(annot_int$cell_id, gsub(" ","",annot_seurat$cell_id)) ]


anocol <- geco.unsupervised::geco.annotToCol4(annotS=annot_int,annotT=annot_int,plotLegend=F,plotLegendFile=file.path(resSUBdir,"Annotation_legends.pdf"), scale_q = "inferno")
...
samples_PDX = c("BC408_chemonaive", "BC408_persister")
...
indice <- which(colnames(anocol)=="cell_cycle")
anocol[,indice] <- as.character(corres_cell_cycle$color[match(annot_int$cell_cycle,corres_cell_cycle$phase)])

corres_cluster <- data.frame(phase=c("C1","C2","C3","C4","C5","C6","C7"),color=c("#6a8caf","#e2979c","#b9cced","#438a5e", "#c0ebb0ff","#fd895eff","#fa6e69ff"))
indice <- which(colnames(anocol)=="clustCol5")
# anocol[,indice] <- as.character(corres_cluster$color[match(annot_int$clustCol5,corres_cluster$phase)])
indice <- which(colnames(anocol)=="louvain_cluster")
...
png(file.path(resSUBdir,"Legend_cell_cycle_scRNAseq.png"), height=2000,width=1500,res=300)
barplot(rep(1,3),col=corres_cell_cycle$color, cex.names  =0.8,horiz=F,names.arg=corres_cell_cycle$phase,las=2)
dev.off()
...
test <- wilcox.test(annot_int$rRNA[which(annot_int$sample_id %in% c("BC408_persister"))],
                    annot_int$rRNA[which(annot_int$sample_id == "BC408_chemonaive")]) 
...
for(j in 1:ncol(anocol))
{
    ...
    if(class(annot_int[1,colnames(anocol)[j]])=="numeric"){
        plot((umap_res), col=alpha(anocol[,j],0.7),pch=20,cex=0.6,
...
load(file.path(resSUBdir,"RData","PDX_BC408.RData"))
load(file.path(resSUBdir,"RData","annot_int.RData"))
RawCounts = Signal
gc()
metadata = annot_int
...
mygps <- list(
    'persister'=metadata[which(metadata$louvain_cluster %in% c("C3") ),"cell_id"]
)
sapply(mygps,length)

myrefs <- list(
    'chemonaive'=metadata[which(metadata$louvain_cluster %in% c("C2") ),"cell_id"]
)
    ...
    for(i in 1:length(myrefs)){
        ...
        WriteXLS(c("over_res","under_res","my.res"),
                 ExcelFileName = file.path(
                     tabdir,paste0("Differential_analysis_Limma_logFC_Pers._vs_",ref,"_",
...
diff_genes <-  c("NNMT","TAGLN","INHBA","KRT6A","KRT6B","KRT16","KRT14","KLK10",
                 "KLK5","KLK7")
diff_genes_figures <- read.csv(file.path(maindir,"output/scRNAseq/PDX_BC976/CrossAnalysis_PDX","bivalent_persister_BC408.csv"))  
diff_genes_figures = diff_genes_figures$x
    unique(c(diff_genes,"KRT17", "INHBB", "KRT5","KRT8","BMP6","CDKN2B","TGFBR2",
