...
```{r paths}
#Paths
resdir <- file.path(here(),"output","scRNAseq","PDX")
...
RawCounts = Signal
gc()
RawCounts_sub = RawCounts[,annot_int_sub$cell_id]
gc()
metadata = annot_int_sub
...
for(i in 1:length(groups)) 	{
    for(log2FC_threshold in log2FC_thresholds){
        ...
        WriteXLS(c("Overexpressed", "Underexpressed"), ExcelFileName = file.path(EnrichDir,paste0("Enrichment_test_",gp,"_vs_",ref,"_",annotbase,"_logFC",round(log2FC_threshold,2),".xlsx")), SheetNames = c( paste0("Overexp_in_", gp), paste0("Underexp_in_", gp) ), perl = "perl", verbose = FALSE, row.names = FALSE, col.names = TRUE, AdjWidth = TRUE, AutoFilter = TRUE, BoldHeaderRow = TRUE, na = "", FreezeRow = 1, FreezeCol = 1)
