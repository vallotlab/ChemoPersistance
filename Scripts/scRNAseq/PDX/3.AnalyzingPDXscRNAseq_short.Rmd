...
samples_PDX = c("HBCx95_v3_UNT","HBCx95_m9_50_REC","HBCx95_persister_2souris","HBCx95_persister_6souris","HBCx95_m30_CAPAR","HBCx95_m6_CAPAS")
...
anocol[,indice] <- as.character(corres_cluster$color[match(annot_int$louvain_partition,corres_cluster$phase)])
...
test <- wilcox.test(annot_int$rRNA[which(annot_int$sample_id %in% c("HBCx95_persister_6souris"))],
                    annot_int$rRNA[which(annot_int$sample_id == "HBCx95_v3_UNT")]) 
median(annot_int$rRNA[which(annot_int$sample_id %in% c("HBCx95_persister_6souris"))])
median(annot_int$rRNA[which(annot_int$sample_id %in% c("HBCx95_v3_UNT"))])
...
geco.hclustAnnotHeatmapPlot(x=cor(mat.so.cor),
                            hc=hc_PCA,
                            hmColors=corColors,
                            anocol=anocol[hc_PCA$order,],#[,ncol(cc.col):1]
...
load(file.path(resSUBdir,"RData","RawCounts_features.RData"))
...
for(log2FC_threshold in log2FC_thresholds){
  for(i in 1:length(myrefs)){
    ref = names(myrefs)[i]
    logFC_name = (paste0("log2FC.persister_6_vs_UNT",ref))
    qval_name = (paste0("qval.persister_6_vs_",ref))
    count_name = (paste0("logCPM.persister_6_vs_",ref))
