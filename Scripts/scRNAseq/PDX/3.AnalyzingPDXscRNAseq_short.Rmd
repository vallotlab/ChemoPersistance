...
if(!(file.exists(file.path(RDatadir,"NormCounts.RData")) & file.exists(file.path(RDatadir,"LogCounts.RData")))){
    ...
    annot_sel2$louvain_cluster <- paste0("C",annot_sel2$louvain_cluster)
...
for(log2FC_threshold in log2FC_thresholds){
    ...
    for(i in gene_lists) {
        ...
        annot_sel2$test <- apply(LogCounts[which(rownames(LogCounts) %in% genes),],2,mean)
        colnames(annot_sel2)[dim(annot_sel2)[2]] <- paste0(i,"_logFC",round(log2FC_threshold,2))
    }
}
...
if(file.exists(file.path(here(), "output","scRNAseq","common_overexpressed_genes_pers_vs_unt.RData"))){
    load(file.path(here(), "output","scRNAseq","common_overexpressed_genes_pers_vs_unt.RData"))
...
anocol2 <- geco.unsupervised::geco.annotToCol4(
    annotS=annot_sel2,annotT=annot_sel2,
...
boxplot(annot_sel2$KEGG_TGF_BETA_SIGNALING_PATHWAY_logFC2~annot_sel2$sample_id,las=2)
dev.off()

png(file.path(resdir_boxplots,"Boxplot_KEGG_TGFB_FC1.png"),width=1500,height=1500,res=300)
boxplot(annot_sel2$KEGG_TGF_BETA_SIGNALING_PATHWAY_logFC1~annot_sel2$sample_id,las=2)
dev.off()
...
save(anocol2,annot_sel2,file=file.path(RDataSupdir,"annot_anocol_final.RData")) 
```
