...
QCdir <- "/media/pprompsy/LaCie/InstitutCurie/Z_server/Manuscripts/2020_ChemoTolerance/Raw_analysis/QC"
resdir <- "/media/pprompsy/LaCie/InstitutCurie/Z_server/Manuscripts/2020_ChemoTolerance/Raw_analysis/scRNAseq/Results_PDX/"
...
# pca_object <- marrow[['pca']]@cell.embeddings
# umap_res <- marrow[['umap']]@cell.embeddings
...
# anocol <- geco.annotToCol3(annotS=annot_sel2[,annotCol],annotT=annot_sel2,plotLegend=T,plotLegendFile=file.path(resSUBdir,"Annotation_legends.pdf"), categCol=NULL)
...
##Association of annotation to partition
# annotCol2 <- c("sample_id","total_features","cell_cycle","rRNA","MKI67","KRT14","NNMT")
# 
# maxKHC <- 15
# for(k in 2:maxKHC)
# {
# 	if(k==2){pval <- geco.groupsVSannot(groups=cutree(hc_PCA,k),annotS=annot_sel2[,annotCol2],annotT=annot_sel2)$pval[,c("anotype","pval")]}else{
# 		pval <- data.frame(pval,geco.groupsVSannot(groups=cutree(hc_PCA,k),annotS=annot_sel2[,annotCol2],annotT=annot_sel2)$pval[,"pval"])
# 	}
# }
# colnames(pval)[-1] <- paste("k",2:maxKHC,sep="")
# pval <- data.frame(Annot=rownames(pval), pval)
# WriteXLS(pval,file.path(resSUBdir,"Hierarchical_clustering_vs_annot.xls"),row.names = T)
# 
# 
# pdf(file.path(resSUBdir,"Hierarchical_clustering_vs_annot.pdf"))
# par(mai=c(1,2,1,1))
# geco.imagePval(t(pval[,-1]),xlab.cex=1,ylab.cex=1)
# dev.off()
```
...
#annotCol <- c("sample_id","total_features","cell_cycle","rRNA","clustCol10","MKI67",diff_genes)
...
# annotCol <- c("sample_id","louvain_partition","total_features","clustCol10","cell_cycle","hallmark_EMT","hallmark_IFN","KRT_signature","hallmark_apoptosis","hallmark_TNFA","NRG1_signalling","MYB_targets","SMID_LUMINAL_DN","TMEM45A","INHBA","TAGLN","NNMT","KRT6A","KRT6B","KRT14","KRT16","KLK10","IL6","MKI67","ARID5B","JUND","KRT17","JUN","FOSL1","AQP3","KLK5","KDM6A","KDM6B","EZH2","CDH1")
...
annot_sel2$ELN <- LogCounts["ELN",]; gc()
#TGFB
annot_sel2$SMAD2 <- LogCounts["SMAD2",]; gc()
annot_sel2$TGFB2 <- LogCounts["TGFB2",]; gc()
annot_sel2$TGFBR3 <- LogCounts["TGFBR3",]; gc()
annot_sel2$INHBB <- LogCounts["INHBB",]; gc()
annot_sel2$INHBA <- LogCounts["INHBA",]; gc()
annot_sel2$TGFBR2 <- LogCounts["TGFBR2",]; gc()
annot_sel2$TGFB1 <- LogCounts["TGFB1",]; gc()
annot_sel2$CDKN2B <- LogCounts["CDKN2B",]; gc()
annot_sel2$BMP6 <- LogCounts["BMP6",]; gc()

annot_sel2$ARID5B <- LogCounts["ARID5B",]; gc()
annot_sel2$JUN <- LogCounts["JUN",]; gc()
annot_sel2$FOSL1 <- LogCounts["FOSL1",]; gc()
annot_sel2$AQP3 <- LogCounts["AQP3",]; gc()
annot_sel2$EZH2 <- LogCounts["EZH2",]; gc()
annot_sel2$KDM6A <- LogCounts["KDM6A",]; gc()
annot_sel2$KDM6B <- LogCounts["KDM6B",]; gc()
annot_sel2$CDH1 <- LogCounts["CDH1",]; gc()

annot_sel2$KRT17 <- LogCounts["KRT17",]; gc()

annot_sel2$MKI67 <- LogCounts["MKI67",]; gc()
annot_sel2$KRT14 <- LogCounts["KRT14",]; gc()
annot_sel2$KRT6A <- LogCounts["KRT6A",]; gc()
annot_sel2$KRT6B <- LogCounts["KRT6B",]; gc()
annot_sel2$KRT16 <- LogCounts["KRT16",]; gc()
annot_sel2$KLK10 <- LogCounts["KLK10",]; gc()
annot_sel2$KLK5 <- LogCounts["KLK5",]; gc()

annot_sel2$TMEM45A <- LogCounts["TMEM45A",]; gc()
annot_sel2$NNMT <- LogCounts["NNMT",]; gc()
annot_sel2$INHBA <- LogCounts["INHBA",]; gc()
annot_sel2$TAGLN <- LogCounts["TAGLN",]; gc()
annot_sel2$MIF <- LogCounts["MIF",]; gc()

load("../common_over_genes_pers_vs_unt.RData")
for(i in common_over_genes){
  annot_sel2[,i] = LogCounts[i,annot_sel2$cell_id]
}
annotCol = unique(c(annotCol, common_over_genes))

ribo <- grepl("^RP[SL]",gene_metadata$Symbol)
annot_sel2$rRNA <- apply(LogCounts[ribo,],2,mean)

library(geco.unsupervised)
anocol2 <- geco.unsupervised::geco.annotToCol4(annotS=annot_sel2[,annotCol],annotT=annot_sel2,plotLegend=T,plotLegendFile=file.path(resSUBdir,"Annotation_legends.pdf"),scale_q = "inferno")
