...
```{r paths}
#Paths
resdir <- file.path(here(),"output","scRNAseq","PDX_BC976")
...
```{r loads}
load(file.path(RDatadir,"PDX.RData"))
annotationDatabases <- c("MSigDB") # "KEGG" or "GO" or "KEGG_GO" or "MSigDB"

sel = (annot$sample_id != "HBCx95_responder" )
annot = annot[sel,]
Signal = Signal[,sel]
...
VlnPlot(marrow, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), ncol = 3)
...
DimPlot(marrow, group.by = "orig.ident",reduction = "umap")
FeaturePlot(marrow, features = c("KDM6B","IGFBP3","KRT6A","KRT14"), pt.size = 0.2)
FeaturePlot(marrow, features =c("PPARG","FOSL1","THAP1","THAP11"), pt.size = 0.2)
FeaturePlot(marrow, features =c("NFKB1","PBX1","EGR4","SMARCC1"), pt.size = 0.2)
FeaturePlot(marrow, features =c("FOSB","JUNB","JUND","JUN"), pt.size = 0.2)
FeaturePlot(marrow, features =c("KRTDAP","KRT6A", "KRT75", "NNMT"), pt.size = 0.2)
FeaturePlot(marrow, features =c("FOSL1","FOSL2"), pt.size = 0.2)
...
cds <- new_cell_data_set(Signal[,rownames(annot_int)],
...
nclust <- 5
annot_int$clustCol5 <- paste("C",match(cutree(hc_PCA,nclust),unique(cutree(hc_PCA,nclust)[hc_PCA$order])),sep="")
nclust <- 6
annot_int$clustCol6 <- paste("C",match(cutree(hc_PCA,nclust),unique(cutree(hc_PCA,nclust)[hc_PCA$order])),sep="")
nclust <- 4
annot_int$clustCol4 <- paste("C",match(cutree(hc_PCA,nclust),unique(cutree(hc_PCA,nclust)[hc_PCA$order])),sep="")
```
...
annotCol <- c("sample_id","total_features","cell_cycle","rRNA","clustCol4",
              "clustCol5","clustCol6","louvain_partition", "louvain_cluster",
              "Seurat_cluster")
pcaText <- FALSE
annotText <- "sample_id"
...
anocol <- geco.unsupervised::geco.annotToCol4(annotS=annot_int[,annotCol],annotT=annot_int,plotLegend=T,plotLegendFile=file.path(resSUBdir,"Annotation_legends.pdf"), scale_q = "inferno")
...
samples_PDX = c("HBCx95_chemonaive", "HBCx95_recurrent", "HBCx95_residual", "HBCx95_persister",   "HBCx95_resistant")
corres <- data.frame(PDX=samples_PDX,color=color_PDX)
anocol[,"sample_id"] <- as.character(corres$color[match(annot_int$sample_id,corres$PDX)])
...
corres_cluster <- data.frame(phase=c("C1","C2","C3","C4"),color=c("#6a8caf","#e2979c","#b9cced","#438a5e"))
...
anocol[,indice]  <- as.character(corres_cluster$color[match(annot_int$louvain_cluster,corres_cluster$phase)])


png(file.path(resSUBdir,"Legend_sample_scRNAseq.png"), height=2000,width=1500,res=300)
barplot(rep(1,5),col=corres$color, cex.names  =0.8,horiz=F,names.arg=corres$PDX,las=2)
dev.off()

png(file.path(resSUBdir,"Legend_cluster_scRNAseq.png"), height=2000,width=1500,res=300)
barplot(rep(1,4),col=corres_cluster$color, cex.names  =0.8,horiz=F,names.arg=c("C1","C2","C3","C4"),las=2)
dev.off()
...
test <- wilcox.test(annot_int$rRNA[which(annot_int$sample_id %in% c("HBCx95_persister"))],
                    annot_int$rRNA[which(annot_int$sample_id == "HBCx95_chemonaive")]) 
median(annot_int$rRNA[which(annot_int$sample_id %in% c("HBCx95_persister"))])
median(annot_int$rRNA[which(annot_int$sample_id %in% c("HBCx95_chemonaive"))])
...
for(j in 1:ncol(anocol))
  ...
  if(class(annot_int[1,colnames(anocol)[j]])=="numeric"){
    plot((umap_res), col=alpha(anocol[,j],0.3),pch=20,cex=0.6,
         main=paste0(colnames(anocol)[j]," min=",round(min(annot_int[,colnames(anocol)[j]]),digits=3)," max=",round(max(annot_int[,colnames(anocol)[j]]),digits=3)),
         xlab="component 1",ylab="component 2")} else {
             plot((umap_res), col=alpha(anocol[,j],0.3),pch=20,cex=0.6,
         main=paste0(colnames(anocol)[j]),
         xlab="component 1",ylab="component 2")
             if(colnames(anocol)[j]=="sample_id"){
                 #Plot persister with add plot since there is too low cell number
                 pers = which(annot_int$sample_id=="HBCx95_persister")
                 points(umap_res[pers,], col=alpha(anocol[pers,j],0.5),pch=20,cex=0.6)
...
# Subsample 
scExp = create_scExp(LogCounts,annot_int[,1:4])
set.seed(47)
scExp = subsample_scExp(scExp,n_cells = 500)
umap_res_sub = umap_res[scExp$cell_id,]
anocol_sub = anocol[scExp$cell_id,]
annot_int_sub = annot_int[scExp$cell_id,]
...
for(j in 1:ncol(anocol_sub))
  ...
  if(class(annot_int_sub[1,colnames(anocol_sub)[j]])=="numeric"){
    plot((umap_res_sub), col=alpha(anocol_sub[,j],0.3),pch=20,cex=0.6,
...
png(file.path(resSUBdir_heatmaps,"Clustering_correlation_matrix_PCA_all_annot.png"), height=1500,width=1500,res=300)
...
ClusterMembership <- as.data.frame.matrix(table(annot_int$sample_id,annot_int$louvain_cluster))
ClusterMembership$pvalue <- 0
ClusterMembership$sample_id <- rownames(ClusterMembership)
for(i in c(1:(dim(ClusterMembership)[1]))){
 test <- fisher.test(ClusterMembership[c(i,1),c(1:5)]) 
...
ClusterMembership$sample_id <- rownames(ClusterMembership)
ClusterMembership_b <- ClusterMembership[,-5] %>% tidyr::pivot_longer(cols=c("C1","C2","C3","C4"),names_to = "cluster",values_to = "freq") %>% dplyr::mutate(cluster=factor(cluster, levels =c("C3","C1","C2","C4"))) %>%
  dplyr::mutate(sample_id=factor(sample_id, levels =c(as.character(corres$PDX[c(1,3,4,2,5)])))) %>%
  dplyr::group_by(sample_id) %>% dplyr::mutate(freq_prop=freq/sum(freq))

png(file.path(resSUBdir_boxplots,"ClusterMembership.png"),width=1000,height=1200,res=300)
...
```{r , out.width="30%",include=FALSE}
cor_mat = cor(t(pca_object))
intra_corr=data.frame()
for(i in unique(annot_int$sample_id)){
    cells = annot_int$cell_id[which(annot_int$sample_id==i)]
    tmp = cor_mat[cells,cells]
    tab = data.frame("sample_id" = rep(i,ncol(tmp)), "intra_corr" = colMeans(tmp))
    intra_corr=rbind(intra_corr,tab)
}

intra_corr$sample_id = factor(intra_corr$sample_id, levels =samples_PDX)

png(file.path(resSUBdir_boxplots,paste0("Sample_intracorrelation_violin.png")),height=1000,width=1000,res=300)
ggplot(intra_corr,aes(x=sample_id,y=intra_corr, fill=sample_id)) + 
  geom_violin(alpha=0.8) + theme_classic() + scale_fill_manual(values=as.character(corres$color)[c(1,2,4,3,5)]) +
...
load(file.path(resSUBdir,"RData","all_sub_500.RData"))
load(file.path(resSUBdir,"RData","PDX.RData"))
RawCounts = Signal
gc()
annot_sel2_sub$cell_id = paste0(annot_sel2_sub$sample_id,"_",annot_sel2_sub$barcode)
RawCounts_sub = RawCounts[,annot_sel2_sub$cell_id]
gc()
metadata = annot_sel2_sub
feature = gene_metadata

table(metadata$louvain_cluster, metadata$sample_id)
#Get the gene list of Persister cells (6souris) overexpressed
mygps <- list(
'persister_vs_UNT'=metadata[which(metadata$sample_id %in% c("HBCx95_persister") ),"cell_id"]
# ,
# 'C2_vs_UNT'=metadata[which(metadata$louvain_partition %in% c("C2") ),"cell_id"],
# 'C3_vs_UNT'=metadata[which(metadata$louvain_partition %in% c("C3") ),"cell_id"],
# 'C4_vs_UNT'=metadata[which(metadata$louvain_partition %in% c("C4") ),"cell_id"]
)
sapply(mygps,length)

myrefs <- list(
    'UNT'=metadata[which(metadata$sample_id %in% c("HBCx95_chemonaive") ),"cell_id"]
    # ,
    # 'UNT'=metadata[which(metadata$sample_id %in% c("HBCx95_chemonaive") ),"cell_id"],
    # 'UNT'=metadata[which(metadata$sample_id %in% c("HBCx95_chemonaive") ),"cell_id"],
    # 'UNT'=metadata[which(metadata$sample_id %in% c("HBCx95_chemonaive") ),"cell_id"]
...
for(log2FC_threshold in log2FC_thresholds){
  for(i in 1:length(myrefs)){
    ref = names(myrefs)[i]
    logFC_name = (paste0("log2FC.persister_vs_", ref))
    qval_name = (paste0("qval.persister_vs_", ref))
    count_name = (paste0("logCPM.persister_vs_", ref))
...
load(file=file.path(RDatadir,"annot_int.RData"))
load(file=file.path(RDataSupdir,"Supervised_res_object_edgeR.Rdata"))
annot_int_2 = annot_int

diff_genes_figures <-  c("GRHL3", "FOXQ1", "IRX5", "EHF", "AHR", "TEAD3")
...
seq = c(14000,12000,16000,14000,12000,16000,14000,16000,16000,14000)- 2000
...
annotCol = unique(c(diff_genes_figures))

for(log2FC_threshold in log2FC_thresholds){
    overgenes = my.res$Symbol[which(my.res$log2FC.persister_6_vs_UNT > log2FC_threshold & 
              my.res$qval.persister_6_vs_UNT < Signif_threshold)]
    for(i in gene_lists) {
        genes <- intersect(overgenes, unlist(str_split(MSIG.gs[which(MSIG.gs$Gene.Set==i),3],",")))
        annot_int_2$test <- apply(LogCounts[which(rownames(LogCounts) %in% genes),],2,mean)
        colnames(annot_int_2)[dim(annot_int_2)[2]] <- paste0(i,"_logFC",round(log2FC_threshold,2))
        annotCol = unique(c(annotCol,colnames(annot_int_2)[dim(annot_int_2)[2]]))
    }
}
...
if(file.exists(file.path(here(), "output","scRNAseq","MM468_PDX","common_over_genes_pers_vs_unt_log2FC1.58.RData"))){
    ...
    for(i in common_overexpressed_genes){
        annot_int_2[,i] <- LogCounts[which(rownames(LogCounts)==i),]
    }
...
anocol2 <- geco.unsupervised::geco.annotToCol4(
    ...
    plotLegend=F, plotLegendFile = file.path(resSUBdir,"Annotation_legends.pdf"),
    scale_q = "inferno")
    ...
    plot((umap_res), col=alpha(anocol2[,j],0.3),pch=20,cex=0.6,
         ...
         xlab="component 1",ylab="component 2")
             if(colnames(anocol2)[j]=="sample_id"){
                 #Plot persister with add plot since there is too low cell number
                 pers = which(annot_int_2$sample_id=="HBCx95_persister_6souris")
