...
diff_genes_figures <- c("FOSL1","TFCP2L1")
...
anocol2 <- geco.unsupervised::geco.annotToCol4(annotS=annot_int,plotLegend = F, scale_q = "inferno")
...
CMH_I_genes <-  c("HLA-A","HLA-B","HLA-C","TAP1","TAP2","PSMB8","PSMB9","NLRC5",
                  "B2M","TAPBP")
LogCounts_chemonaive = LogCounts[,grep("chemonaive",colnames(LogCounts))]
annot_int_chemonaive = annot_int[which(annot_int$sample_id == "MM468_chemonaive"),]

CMH_values =rowMeans(LogCounts_chemonaive[match(CMH_I_genes,rownames(LogCounts_chemonaive)),])
Other_genes_values = rowMeans(LogCounts_chemonaive[-match(CMH_I_genes,rownames(LogCounts_chemonaive)),])

png(file.path(resdir_boxplots,paste0("Boxplot_CMH_I_genes_vs_other_expressed_genes.png")),width=2000,height=1200,res=300)
hist(Other_genes_values, breaks= 150, main =  "Gene Log-Expression distribution", xlab = "LogExpression") 
seq = c(14000,12000,16000,14000,12000,16000,14000,16000,16000,14000)
names(seq) = CMH_I_genes
for(i in CMH_I_genes){
abline(v=CMH_values[i], col ="red", lty = 2, lwd = 0.5)
text(x = CMH_values[i]+0.25, y = seq[i], label=i, col = "red", cex = 0.35)
}
dev.off()
