...
rdatadir <- file.path(resdir, "Unsupervised","RData")
...
```{r eval=FALSE}
### Load data
load(file.path(rdatadir,"UNC_RawCounts.RData"))
load(file.path(rdatadir,"annot_int.RData"))

exprMat <- RawCounts
cellInfo <- annot_int

cellInfo = cellInfo[which(cellInfo$sample_id %in% c("MM468_initial","MM468_UNC_day33")),]

# Subsample to 300 each
set.seed(47)
cells = c(sample(which(cellInfo$sample_id == "MM468_initial"),300), sample(which(cellInfo$sample_id == "MM468_UNC_day33"),300))
...
dbs = db_names
db_mcVersion <- 'v9'
db_path <- file.path(maindir,"annotation","CisTarget")

scenicOptions <- initializeScenic(org="hgnc", dbDir=file.path(maindir,"annotation","CisTarget"), 
                                  dbs = db_names, nCores=2)
scenicOptions@settings$dbs <- db_names
...
pdf(file.path(output,"UNC_rssPlot.pdf"))
rssPlot$plot
dev.off()
...
tf_UNC = read.table("output/Step2_MotifEnrichment.tsv", sep = "\t", header = TRUE)
# Retrieve FOS & FOSL1 targets, as found by SCENIC
TF_genes = read.table("output/Step2_regulonTargetsInfo.tsv", header = TRUE)
