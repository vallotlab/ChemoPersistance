...
maindir= here()
resdir <- file.path(maindir,"output","scRNAseq","MM468","UNC","Supervised")
...
RDatadir <- file.path(file.path(maindir,"output","scRNAseq","MM468","UNC","Unsupervised","RData"))

source(file.path(maindir,"Scripts","global_var.R"))

## Import annotation file
load(file.path(RDatadir,"annot_int.RData"))
load(file.path(resdir,"..","..","Persister","Unsupervised","RData","MM468.RData"))
...
if ("edgeR" %in% algoType) {
    ...
    for(log2FC_threshold in log2FC_thresholds){
        under_res = my.res %>% dplyr::filter(log2FC.UNC < -log2FC_threshold & qval.UNC < Signif_threshold) %>% 
            dplyr::arrange(qval.UNC) %>% dplyr::select(Symbol,log2FC.UNC,qval.UNC)
        over_res = my.res %>% dplyr::filter(log2FC.UNC > log2FC_threshold & qval.UNC < Signif_threshold) %>% 
            dplyr::arrange(qval.UNC) %>% dplyr::select(Symbol,log2FC.UNC,qval.UNC)
...
for(log2FC_threshold in log2FC_thresholds){
    overgenes = my.res$Symbol[which(my.res$log2FC.UNC > log2FC_threshold & 
                                        my.res$qval.UNC < Signif_threshold)]
    for(i in gene_lists) {
        genes <- intersect(overgenes, unlist(str_split(MSIG.gs[which(MSIG.gs$Gene.Set==i),3],",")))
        annot_int$test <- apply(UNC_LogCounts[which(rownames(UNC_LogCounts) %in% genes),],2,mean)
...
for(log2FC_threshold in log2FC_thresholds){
    ...
    for(i in 1:dim(significPathway)[1]){
        ...
        mat <- UNC_LogCounts[gene_metadata$Symbol %in% gene_list,]
