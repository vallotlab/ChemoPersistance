...
scRNA = list()
for(file in list.files(GEOdir_scRNA_MM468,pattern = ".*UNC.*21.*_barcodes.tsv.gz", full.names = TRUE )){
...
for(i in seq_along(scRNA)){
  ...
  lineage_AM <- read.csv(file.path(GEOdir_scRNA_MM468,"barcoding_output",
                                   paste0("consensus_cell_10xbc_and_vbc_",name,".csv")))
  lineage_AM$BC_10x <- substr(lineage_AM$BC_10x,0,16)
  
  colnames(metadata)[1] <- "BC_10x"
  metadata <- left_join(metadata,lineage_AM)
