...
# Directories -------------------------------------------------------------
outdir = file.path(maindir, "output","scRNAseq", "HCC38");  if(!dir.exists(outdir)){dir.create(outdir)}
...
# Replace GEO path with the path to the GEO directory downloaded on your computer (scRNA HCC38)
GEOdir_scRNA_HCC38 = file.path(maindir,"input","scRNAseq","MM468") 

# Load Data ---------------------------------------------------------------
BCmodel <- "HCC38"

scRNA = list()
for(file in list.files(GEOdir_scRNA_HCC38,pattern = "HCC38.*_barcodes.tsv.gz", full.names = TRUE )){
