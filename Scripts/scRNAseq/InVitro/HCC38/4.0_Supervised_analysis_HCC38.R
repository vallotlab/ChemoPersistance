...
maindir= here()
resdir <- file.path(maindir,"output","scRNAseq","HCC38","Persister","Supervised")
...
RDatadir <- file.path(file.path(maindir,"output","scRNAseq","HCC38","Persister","Unsupervised","RData"))
...
# mygps <- list(
#     'persister'=metadata[which(metadata$sample_id %in% c("HCC38_persister") ),"cell_id"]
# )
# myrefs <- list(
#     'chemonaive'=metadata[which(metadata$sample_id %in% c("HCC38_chemonaive") ),"cell_id"]
# ) # Sets reference (can be 1 or more samples)

mygps <- list(
    'persister'=metadata[which(metadata$louvain_cluster %in% paste0("C",c(1,3,5,8)) ),"cell_id"]
)
myrefs <- list(
    'chemonaive'=metadata[which(metadata$louvain_cluster %in% paste0("C",c(2,4,6,7)) ),"cell_id"]
) # Sets reference (can be 1 or more samples)
            ...
            WriteXLS(
                ...
                ExcelFileName = file.path(EnrichDir,
                                          paste0("Enrichment_test_",gp,"_vs_",ref,"_",annotbase,
                                                 "_logFC",round(log2FC_threshold,2),"_persisterC1_3_5_8_vs_chemonaive_C2_4_6_7.xlsx")), 
...
for(i in "CDH2")#setdiff(colnames(anocol2),colnames(anocol)))
{
