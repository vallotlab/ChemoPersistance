...
res_folder_name <- "All_clusters_vs_other_clusters__edgeR"
...
resdir <- "/media/pprompsy/LaCie/InstitutCurie/Z_server/Manuscripts/2020_ChemoTolerance/Raw_analysis/scRNAseq/Results_MM468/Supervised_persister";if(!file.exists(resdir)){dir.create(resdir)}
...
RDatadir <- "/media/pprompsy/LaCie/InstitutCurie/Z_server/Manuscripts/2020_ChemoTolerance/Raw_analysis/scRNAseq/Results_MM468/Unsupervised_persister/RData/"
...
mygps <- list(
'persisterD33'=metadata[which(metadata$sample_id %in% c("MM468_5FU6_day33") ),"cell_id"],
'persisterall'=metadata[which(metadata$sample_id %in% c("MM468_5FU6_day33","MM468_5FU3_day50","MM468_5FU3_day77","MM468_5FU5_day67") ),"cell_id"],
'C2_C4_pers'=metadata[which(metadata$louvain_partition %in% c("C2","C4") ),"cell_id"],
'C2_pers'=metadata[which(metadata$louvain_partition %in% c("C2") ),"cell_id"]
# 'C3_per2'=metadata[which(metadata$louvain_partition == "C3" ),"cell_id"],
# 'C5_per3'=metadata[which(metadata$louvain_partition == "C5" ),"cell_id"]#,
# 'C2_res5FU5'=metadata[which(metadata$louvain_partition == "C2" ),"cell_id"],
# 'C4_res5FU3'=metadata[which(metadata$louvain_partition == "C4" ),"cell_id"]
)
...
'DMSO'=metadata[which(metadata$sample_id %in% c("MM468_initial") ),"cell_id"],
'C6_DMSO'=metadata[which(metadata$louvain_partition == "C10" ),"cell_id"],
'DMSO'=metadata[which(metadata$sample_id %in% c("MM468_initial") ),"cell_id"]
# 'C6_DMSO'=metadata[which(metadata$louvain_partition == "C10" ),"cell_id"],
# 'C6_DMSO'=metadata[which(metadata$louvain_partition == "C10" ),"cell_id"]#,
# 'C1_per1'=metadata[which(metadata$louvain_partition == "C1" ),"cell_id"],
# 'C3_per2'=metadata[which(metadata$louvain_partition == "C3" ),"cell_id"]
...
if ("edgeR" %in% algoType) {
  ...
  under_res = my.res %>% dplyr::filter(log2FC.C2_pers < -FC_threshold & qval.C2_pers < Signif_threshold) %>% 
    dplyr::arrange(qval.C2_pers) %>% select(Symbol,log2FC.C2_pers,qval.C2_pers)
  over_res = my.res %>% dplyr::filter(log2FC.C2_pers > FC_threshold & qval.C2_pers < Signif_threshold) %>% 
    dplyr::arrange(qval.C2_pers) %>% select(Symbol,log2FC.C2_pers,qval.C2_pers)
