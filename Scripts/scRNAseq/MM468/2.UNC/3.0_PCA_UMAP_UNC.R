...
save(annot_int, gene_metadata,file=file.path(RDatadir,"UNC_gene_cell_annot_louvain.RData"))
################################################################################################
...
for(j in 1:ncol(anocol))
{
  ...
  plot((umap_res[annot_int$cell_id,]), col=alpha(anocol[,j],0.10), pch=20, cex=0.6,
...
if(length(indice) >0 ){
  ...
  plot((umap_res[!is.na(annot_int$cons_BC_lenti),]),
       ...
       pch=20,cex=0.6,main=paste0(colnames(anocol)[indice]," perplexity=30"),xlab="component 1",ylab="component 2",ylim=c(min(umap_res[,2]),max(umap_res[,2])))
  dev.off()
