...
annot_int <- annot_sel[annot_sel$sample_id %in% c("MM468_initial","MM468_UNC_day33","MM468_5FU6_UNC_day33","MM468_5FU6_day33","MM468_5FU3_day50","MM468_5FU5_day67") & annot_sel$doublet_score<10000,]
...
annot_int$KRT14 <- UNC_LogCounts["KRT14",annot_int$cell_id]; gc()
annot_int$KRT17 <- UNC_LogCounts["KRT17",annot_int$cell_id]; 
annot_int$KLK10 <- UNC_LogCounts["KLK10",annot_int$cell_id]; gc()
annot_int$MKI67 <- UNC_LogCounts["MKI67",annot_int$cell_id]; gc()
annot_int$NNMT <- UNC_LogCounts["NNMT",annot_int$cell_id]; gc()
annot_int$TGFB1 <- UNC_LogCounts["TGFB1",annot_int$cell_id]; gc()
annot_int$TGFBR1 <- UNC_LogCounts["TGFBR1",annot_int$cell_id]; gc()
annot_int$TGFBR3 <- UNC_LogCounts["TGFBR3",annot_int$cell_id]; gc()
annot_int$TGFBR2 <- UNC_LogCounts["TGFBR2",annot_int$cell_id]; gc()
annot_int$INHBA <- UNC_LogCounts["INHBA",annot_int$cell_id]; gc()
annot_int$INHBB <- UNC_LogCounts["INHBB",annot_int$cell_id]; gc()
annot_int$BMP4 <- UNC_LogCounts["BMP4",annot_int$cell_id]; gc()
annot_int$VIM <- UNC_LogCounts["VIM",annot_int$cell_id]; gc()
annot_int$ABCC4 <- UNC_LogCounts["ABCC4",annot_int$cell_id]; gc()

ribo <- grepl("^RP[SL]",gene_metadata$Symbol)
annot_int$rRNA <- apply(UNC_LogCounts[ribo,],2,mean); gc()

#Generate color annotation & legend
anocol <- geco.annotToCol4(annotS=annot_int[,annotCol],annotT=annot_int,plotLegend=T,plotLegendFile=file.path(resSUBdir,"Annotation_legends.pdf"), categCol=NULL)
...
test <- annot_subset %>% mutate(ExpressionGroup=factor(ExpressionGroup,levels = unique(annot_subset$ExpressionGroup))) %>%
  mutate(sample_id=factor(sample_id,levels=c("MM468_5FU3_day50","MM468_5FU5_day67","MM468_5FU6_day33","MM468_5FU6_UNC_day33",
                                             "MM468_initial","MM468_UNC_day33"))) %>% 
...
png(file.path(resSUBdir,paste0("Re_Clustering_MostVarying_",NbGenes,"_",distHC,"_",methHC,".png")), height=4000,width=4000,res=300)
...
for(Sample in unique(annot_subset$sample_id)){
  ...
  png(file.path(resSUBdir,paste0("Pearson_Correlation_scores_",Sample,".png")), height=1000,width=1000,res=300)
  image((as.matrix(MAT[,-1])),col=rev(viridis(100)))
  dev.off()
  png(file.path(resSUBdir,paste0("Distrib_Pearson_Correlation_scores_",Sample,".png")), height=1000,width=1000,res=300)
...
png(file.path(resSUBdir,paste0("DistribPearsonCorrelationScores.png")),height=1000,width=1500,res=300)
