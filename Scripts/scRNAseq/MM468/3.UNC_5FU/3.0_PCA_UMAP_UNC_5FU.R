...
sample_persisters_study = c(
  ...
  "MM468_5FU1_day214","MM468_5FU1_UNC_day33","MM468_UNC_day33"
)

# Select initial population, 4 'persister' states (early) and 3 'resistant' states (late)
sample_UNC_5FU_study = c(
  "MM468_chemonaive","MM468_5FU1_day33","MM468_5FU3_day50","MM468_5FU2_day67", "MM468_5FU1_UNC_day33"
)
...
color_MM468 <- c(control[2],pers_color[1:3],treated_UNC_color[1])
...
if(RECOMPUTE){
  ...
  ################################################################################################
  load(file.path(maindir,"output","scRNAseq","MM468","Persister","Unsupervised","RData","MM468_old.RData"))
  ...
  load(file.path(maindir, "output","scRNAseq","MM468","Persister","Unsupervised","Rdata","persister_gene_cell_annot_old.RData"))
  ...
  barplot(rep(1,5),col=as.character(corres_sample$color), cex.names  =0.8,horiz=F,names.arg=corres_sample$Sample,las=2)
  dev.off()
  
  png(file.path(resdir,"Legend_cluster_scRNAseq.png"), height=2000,width=1500,res=300)
  barplot(rep(1,6),col=as.character(corres_cluster$color), cex.names  =0.8,horiz=F,names.arg=corres_cluster$cluster,las=2)
  dev.off()
...
annot_subset <- annot_int[!is.na(annot_int$lineage_barcode),]
...
geco.hclustAnnotHeatmapPlot(x=cor(mat.so.cor),
                            hc=hc_PCA,
                            hmColors=corColors,
                            anocol=as.matrix(anocol_subset[hc_PCA$order,c(1,4,5,9)]),#[,ncol(cc.col):1]
...
for(i in annotCol){
  if(i %in% rownames(UNC_LogCounts))
    annot_int[,i] <- UNC_LogCounts[which(rownames(UNC_LogCounts)==i),]
}
gc()
...
for(i in setdiff(colnames(anocol2),colnames(anocol)))
{
  ...
  png(file.path(resdir_UMAPs,paste0("UMAP_",colnames(anocol2)[j],".png")), height=1500,width=1500,res=300)
