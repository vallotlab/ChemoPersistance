...
unt_pers_cells = c(mygps$persister, myrefs$UNT) # subsample
...
ggplot(coocurrence_persister_genes_score, aes(coocurrence_score,fill=sample,color=sample)) +
  ...
  scale_fill_manual(values=as.character(unique(anocol.[,"louvain_partition"])[2:1])) +
  geom_vline(xintercept = quantile(coocurrence_persister_genes_score$coocurrence_score[which(coocurrence_persister_genes_score$sample=="Persister_6")],0.25), col=unique(anocol.[,"louvain_partition"])[1],lty=3)
dev.off()
...
for(i in 1:2){
  ...
  genes = overexpressed_pers_genes
  mat <- LogCounts_pers_ini[rownames(LogCounts_pers_ini) %in% genes,cells]
...
set.seed(2047)
results <- rfe(mat, class, sizes=c(3:25,seq(30,97,5)), rfeControl=control)

save(results, file = file.path(RDatadir,"results_rfe_MM468.RData"))
gc()
# OR load results :
...
# Set predictor genes as TOP 10 genes (as in PDX) (top 10 of subsampling = 10 variables)
predictor_genes = unique(results$variables$var[which(results$variables$Variables==10)])[1:10]

genes=overexpressed_pers_genes
mat = LogCounts_pers_ini[rownames(LogCounts_pers_ini) %in% predictor_genes,unt_pers_cells_500]
tmat <- t(mat)
distHC <- c("distPearson","distCosine","euclidean","maximum","manhattan","canberra","binary","minkowski")[3]
methHC <- c("ward","ward.D","ward.D2","single","complete","average")[3]
if(distHC=="distPearson")	d <- distPearson(tmat)
...
house_keeping_genes = (intersect(house_keeping_genes,rownames(LogCounts_pers_ini)))

load(file.path(RDatadir_unsup,"persister_RawCounts.RData"))
...
mat_pers = LogCounts_pers_ini[rownames(LogCounts_pers_ini) %in% c(overexpressed_pers_genes,
...
for(gene_k in rownames(fisher_pvalue_mat)){
  for(gene_i in rownames(fisher_pvalue_mat)){
      if(fisher_pvalue_mat[gene_i,gene_k] %in% top){
        ...
        tab$total_counts = annot_int$total_counts[which(annot_int$cell_id %in% tab$cell_id)]
...
mat <- LogCounts_pers_ini[rownames(LogCounts_pers_ini) %in% common_overexpressed_genes,unt_pers_cells]
bin_mat = mat
bin_mat[(bin_mat>0)]=1

coocurrence_persister_genes_score_common = as.data.frame(colSums(bin_mat) / nrow(bin_mat))
colnames(coocurrence_persister_genes_score_common) = "coocurrence_score"
rownames(coocurrence_persister_genes_score_common) = colnames(bin_mat)
coocurrence_persister_genes_score_common$sample = ""
coocurrence_persister_genes_score_common[mygps$persister,"sample"] = "Persister"
coocurrence_persister_genes_score_common[myrefs$UNT,"sample"] = "Untreated"

coocurrence_persister_genes_score_common$sample = factor(coocurrence_persister_genes_score_common$sample,levels=
                                                    c("Untreated","Persister"))
...
mat = LogCounts_pers_ini[rownames(LogCounts_pers_ini) %in% overexpressed_pers_genes, unt_pers_cells_500]
rownames(mat) = overexpressed_pers_genes
bin_mat =  as.matrix(mat)
bin_mat[bin_mat>0] = 1
dim(bin_mat)
vars_p = apply(bin_mat[,1:500], MARGIN = 1, var)
vars_u = apply(bin_mat[,501:1000], MARGIN = 1, var)
...
mat = LogCounts_pers_ini[rownames(LogCounts_pers_ini) %in% overexpressed_pers_genes, c(mygps$persister,myrefs$UNT)]
bin_mat =  as.matrix(mat)
bin_mat[bin_mat>0] = 1
