...
inputDir_coverage <- file.path(maindir,"input","bulk_ChIPseq","Human", "BigWigs")
...
K27 = sapply(list.files(inputDir_coverage, pattern = "K27"), function(i) rtracklayer::import(file.path(inputDir_coverage, i)))
names(K27) = gsub("hT_PDX_BC|_K27me3.bw", "",list.files(inputDir_coverage, pattern = "K27"))
K4 = sapply(list.files(inputDir_coverage, pattern = "K4"), function(i) rtracklayer::import(file.path(inputDir_coverage, i)))
names(K4) = gsub("hT_PDX_BC|_K4me3.bw", "",list.files(inputDir_coverage, pattern = "K4"))
...
for(i in 1:length(regions)){
    ...
    ## 5FU & DMSO sc K27
    K4_tmp = sapply(K4, function(i) i[subjectHits(findOverlaps(roi,i)),] )
    K27_tmp = sapply(K27, function(i) i[subjectHits(findOverlaps(roi,i)),] )
    
    K4_tmp = sapply(K4_tmp, function(i) {if(length(i) ==0 ) i = roi ; i})
    K27_tmp = sapply(K27_tmp, function(i) {if(length(i) ==0 ) i = roi ; i})
    
    max_K4 = round(max(unlist(sapply(K4_tmp, function(i) (i$score))),2)) 
    max_K27 = round(max(unlist(sapply(K27_tmp, function(i) (i$score))),2))
    
    if(TRUE){
        pdf(file.path(file.path(resDir,samples),paste0(samples,"_",region,".pdf")),width=8,height=6)
        layout.matrix <- matrix(c(unlist(lapply(1:20,function(i) rep(i,3))),21,21,21,22,22), ncol = 1)
