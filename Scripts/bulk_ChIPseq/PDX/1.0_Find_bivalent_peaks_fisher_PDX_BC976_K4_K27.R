...
maindir= here()
resdir <- file.path(maindir,"output","bulk_ChIPseq","BC976","ChIPreChIP")
if(!dir.exists(resdir)) dir.create(resdir)
source(file.path(maindir,"Scripts","global_var.R"))


inputDir =  "/media/pacome/Depic_bioinfo_1/InstitutCurie/Documents/Data/results/ChIP_reChIP_D712/BC976/03subsample_bam///"# file.path(maindir, "input","bulk_ChIPseq","BC976","ChIPreChIP")
outputDir = file.path(maindir, "output","bulk_ChIPseq","BC976","ChIPreChIP")
library(here)

# K27 K4 files
primary_K4 = file.path(inputDir,"BC976_primary_H3K4me3_H3K27me3.subsample.bam")
chipReChip_K4_K27 = file.path(inputDir,"BC976_reChIP_H3K4me3_H3K27me3.subsample.bam")
summits_K4_K27_primary = rtracklayer::import(file.path(maindir, "input","bulk_ChIPseq","PDX","ChIPreChIP","Peaks","BC976_primary_H3K4me3_H3K27me3_summits_merged_filtered_by_TSS.bed"))
summits_K4_K27_primary = unique(summits_K4_K27_primary)

# K27 IgG files
primary_K4_IgG = file.path(inputDir,"BC976_primary_H3K4me3_IgG.subsample.bam")
chipReChip_K4_IgG= file.path(inputDir,"BC976_reChIP_H3K4me3_IgG.subsample.bam")
summits_K4_IgG_primary = rtracklayer::import(file.path(maindir, "input","bulk_ChIPseq","PDX","ChIPreChIP","Peaks","BC976_primary_H3K4me3_IgG_summits_merged_filtered_by_TSS.bed"))
summits_K4_IgG_primary = unique(summits_K4_IgG_primary)

# Summits K27 K4
summits_K4_K27_close = summits_K4_K27_primary

# Close is +- 2500 around summit
...
filt <- Rsamtools:: ScanBamParam(which = summits_K4_K27_close)d
...
write.csv(ratio, file.path(outputDir,"peak_ratios_K4_K27.csv"))
ratio = read.csv(file.path(outputDir,"peak_ratios_K4_K27.csv"))
...
write.csv(ratio, file.path(outputDir,"peak_ratios_K4_IgG.csv"))
...
length(which(fisher_pvalue_IgG_df$V1 < 0.25 & fisher_pvalue_IgG_df$odd_ratio > 2))
length(which(fisher_pvalue_IgG_df$V1 < 0.15 & fisher_pvalue_IgG_df$odd_ratio > 2))
length(which(fisher_pvalue_IgG_df$V1 < 0.1 & fisher_pvalue_IgG_df$odd_ratio > 2))
length(which(fisher_pvalue_IgG_df$V1 < 0.05 & fisher_pvalue_IgG_df$odd_ratio > 2))
length(which(fisher_pvalue_IgG_df$V1 < 0.01 & fisher_pvalue_IgG_df$odd_ratio > 2))

length(which(fisher_pvalue_df$V1 < 0.25 & fisher_pvalue_df$odd_ratio > 2))
length(which(fisher_pvalue_df$V1 < 0.15 & fisher_pvalue_df$odd_ratio > 2))
length(which(fisher_pvalue_df$V1 < 0.1 & fisher_pvalue_df$odd_ratio > 2))
length(which(fisher_pvalue_df$V1 < 0.05 & fisher_pvalue_df$odd_ratio > 2))
length(which(fisher_pvalue_df$V1 < 0.01 & fisher_pvalue_df$odd_ratio > 2))

length(which(fisher_pvalue_IgG_df$fisher.pvalue < 0.25 & fisher_pvalue_IgG_df$odd_ratio > 0.5))
length(which(fisher_pvalue_IgG_df$fisher.pvalue < 0.25 & fisher_pvalue_IgG_df$odd_ratio > 1))
length(which(fisher_pvalue_IgG_df$fisher.pvalue < 0.25 & fisher_pvalue_IgG_df$odd_ratio > 2))
length(which(fisher_pvalue_IgG_df$fisher.pvalue < 0.25 & fisher_pvalue_IgG_df$odd_ratio > 3))
length(which(fisher_pvalue_IgG_df$fisher.pvalue < 0.25 & fisher_pvalue_IgG_df$odd_ratio > 4))
length(which(fisher_pvalue_IgG_df$fisher.pvalue < 0.25 & fisher_pvalue_IgG_df$odd_ratio > 5))
length(which(fisher_pvalue_IgG_df$odd_ratio > 5))

length(which(fisher_pvalue_df$fisher.pvalue < 0.25 & fisher_pvalue_df$odd_ratio > 0.5))
length(which(fisher_pvalue_df$fisher.pvalue < 0.25 & fisher_pvalue_df$odd_ratio > 1))
length(which(fisher_pvalue_df$fisher.pvalue < 0.25 & fisher_pvalue_df$odd_ratio > 2))
length(which(fisher_pvalue_df$fisher.pvalue < 0.25 & fisher_pvalue_df$odd_ratio > 3))
length(which(fisher_pvalue_df$fisher.pvalue < 0.25 & fisher_pvalue_df$odd_ratio > 4))
length(which(fisher_pvalue_df$fisher.pvalue < 0.25 & fisher_pvalue_df$odd_ratio > 5))
...
tab=data.frame()
for(qval in c(0.25,0.2,0.15,0.1,0.05,0.01,0.005,0.001,0.0001,1e-5,1e-10,1e-11,1e-12,1e-13,1e-14,1e-15)){
    for(peak_ratio in c(1,2,3,4,5)){
        # select signif peaks for K27 K4
        fisher_pvalue_df$Significant = (fisher_pvalue_df$qvalue <qval & fisher_pvalue_df$odd_ratio > peak_ratio)
        ...
        fisher_pvalue_IgG_df$Significant = (fisher_pvalue_IgG_df$qvalue<qval & fisher_pvalue_IgG_df$odd_ratio > peak_ratio)
...
ggplot(tab) + geom_point(aes(x = (num_bivalent_peaks), y =false_positive,
    ...
    ylab("# False positive") + xlim(c(0,9000))
dev.off()
...
# fisher_pvalue_df. = fisher_pvalue_df[fisher_pvalue_df$Significant,1,drop=F]
# fisher_pvalue_df.$chr = gsub(":.*","",fisher_pvalue_df.$X)
# fisher_pvalue_df.$start = as.numeric(gsub("-.*","",gsub(".*:","",fisher_pvalue_df.$X)))
# fisher_pvalue_df.$end =  as.numeric(gsub(".*-","",gsub(".*:","",fisher_pvalue_df.$X)))

bivalent_genes = unique(unlist(strsplit(summits_K4_K27_peak_significant$gene,split=",")))
# summits_K4_K27_peak_significant = as(fisher_pvalue_df.[,2:4],"GRanges")
...
WriteXLS(
    ...
    ExcelFileName = file.path(outputDir,
                              paste0("Enrichment_test_Bivalent_pathways_K4_K27.xlsx")), 
