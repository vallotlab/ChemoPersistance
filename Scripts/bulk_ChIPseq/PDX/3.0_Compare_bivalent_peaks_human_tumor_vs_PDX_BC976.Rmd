...
resdir_PDX <- file.path(maindir,"output","bulk_ChIPseq","BC976","ChIPreChIP")
...
bivalent_genes_human_tumor = read.csv(file.path(resdir_human_tumor,"K27_K4_df_hT_PDX_BC976.csv"))
...
bivalent_GSA_human_tumor = readxl::read_xlsx(file.path(resdir_human_tumor,"Enrichment_test_Bivalent_pathways_hT_PDX_BC976.xlsx"))

classes = c("c2_curated","c5_GO","hallmark")
pattern = c("HALLMARK|BREAST|MAMMARY")
...
bivalent_genes_PDX = read.csv(file.path(resdir_PDX,"fisher_pvalue_K4_K27_peak_1e-15_4.csv"))
...
WriteXLS::WriteXLS(GSA_combined_save, ExcelFileName = file.path(resdir_PDX,"combined_GSA_Bivalence_HumanTumor_vs_PDX_BC976.xlsx"))

png(file.path(resdir_PDX,"Intersection_PDX_bivalent_pathways.png"),width = 1200, height = 1200, res = 200)
gplots::venn(
    list("HumanTumor_BC976" = bivalent_GSA_human_tumor$Gene.Set,
         "PDX_BC976" = bivalent_GSA_PDX$Gene.Set),
    show.plot = T)
dev.off()

png(file.path(resdir_PDX,"Intersection_PDX_bivalent_genes.png"),width = 1200, height = 1200, res = 200)
gplots::venn(
    list("HumanTumor_BC976" = bivalent_genes_human_tumor,
         "PDX_BC976" = bivalent_genes_PDX),
    show.plot = T)
dev.off()
...
# Fisher test
annot_10k = read.table(file.path(maindir,"annotation", "gencode.v34.annotation.transcriptTSS_10k.bed"))
all_genes = unique(annot_10k$V5)

A_B = length(intersect(bivalent_genes_human_tumor,bivalent_genes_PDX))
A_nonB = length(bivalent_genes_human_tumor) - A_B
B_nonA =  length(bivalent_genes_PDX) - A_B
Omega = length(all_genes) - length(union(bivalent_genes_human_tumor,bivalent_genes_PDX))
