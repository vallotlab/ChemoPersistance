...
maindir= here()
resdir <- file.path(maindir,"output","bulk_ChIPseq","BT20","ChIPreChIP")
if(!dir.exists(resdir)) dir.create(resdir)
source(file.path(maindir,"Scripts","global_var.R"))


inputDir =  "/media/pprompsy/Depic_bioinfo_1/InstitutCurie/Documents/Data/results/ChIP_reChIP_D712/BT20/03subsample_bam/"# file.path(maindir, "input","bulk_ChIPseq","BT20","ChIPreChIP")
outputDir = file.path(maindir, "output","bulk_ChIPseq","BT20","ChIPreChIP")

# K27 K4 files
# No primary K4 - K27 !!! Take primary K4 - IgG
primary_K4 = file.path(inputDir,"BT20_primary_H3K4me3_IgG.subsample.bam")
chipReChip_K4_K27 = file.path(inputDir,"BT20_reChIP_H3K4me3_H3K27me3.subsample.bam")
summits_K4_K27_primary = rtracklayer::import(file.path(maindir, "input","bulk_ChIPseq","InVitro","ChIPreChIP", "Peaks","H3K4me3_IgG_primary_summits_merged_filtered_by_TSS.bed"))
summits_K4_K27_primary = unique(summits_K4_K27_primary)

# K27 IgG files
primary_K4_IgG = file.path(inputDir,"BT20_primary_H3K4me3_IgG.subsample.bam")
chipReChip_K4_IgG= file.path(inputDir,"BT20_reChIP_H3K4me3_IgG.subsample.bam")
...
scRNA = readxl::read_xlsx(file.path(maindir,"output","scRNAseq","BT20","Persister","Supervised","Tables","Differential_analysis_Limma_logFC_1.58.xlsx"))
tab=data.frame()
...
ggplot(tab) + geom_point(aes(x = (num_bivalent_peaks), y =false_positive,
    ...
    ylab("# False positive") + xlim(c(0,6000)) + ylim(c(0,6000))
dev.off()
